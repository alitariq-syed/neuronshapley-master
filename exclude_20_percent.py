...
while True:
    for adv in adversarials.split(','):
        ...
        for key in keys.split(','):
            ...
            if not tf.io.gfile.exists(os.path.join(cb_dir, 'chosen_players.txt')):
            ...
            thresh = (vals)[np.argsort(vals)[-top_k]]
            # chosen_players = np.where(((vals - cbs) < thresh) * ((vals + cbs) > thresh))[0]
            chosen_players = np.where(((vals ) < thresh) )[0]
            ...
            new_players = np.asarray(new_players)
            open(os.path.join(cb_dir, 'players.txt'), 'w').write(
                ','.join(new_players.astype(str)))
