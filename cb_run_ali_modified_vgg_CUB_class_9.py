...
'''
run for finding class specific critical neurons
1. For a given class, choose class recall as the performance metric
2. Exclude top 20% globally important neurons
    by disabling the players in players.txt using chosen top 4224-845 neurons
3. Find top-k class specific neurons
'''
...
if default:    
    ...
    if COLAB:
      DATA_DIR = './test'#'G:/CUB_200_2011/CUB_200_2011/train_test_split/test'
    elif LAB:
      DATA_DIR = 'D:/Ali Tariq/CUB_200_2011/train_test_split/test'#'G:/CUB_200_2011/CUB_200_2011/train_test_split/test'
    else:
      DATA_DIR = 'G:/CUB_200_2011/CUB_200_2011/train_test_split/test'
    
    if COLAB:
        MEM_DIR = '/content/drive/MyDrive/results_local'
    else:
        MEM_DIR = './results'
...
while True:
    ...
    if COLAB: #get chosenplayer for ./results path but save results in ./local_results on drive
        ss = cb_dir
        cb_dir = './results/NShap/vgg/accuracy_all_all_layer_new/cb_Bernstein_0.2_128'
        assert(False)# fix above path for class specific code
    if tf.io.gfile.exists(os.path.join(cb_dir, 'chosen_players_top_865_disabled.txt')):
        chosen_players = open(os.path.join(
                cb_dir, 'chosen_players_top_865_disabled.txt')).read()
        try:
